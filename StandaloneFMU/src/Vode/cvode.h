...
typedef void (*RhsFn)(integer N, real t, iN_Vector* y, iN_Vector* ydot,
                      void *f_data);
...
typedef struct CVodeMemRec {
  ...
  iN_Vector* cv_zn[L_MAX];  /* Nordsieck array N x (q+1),                  */
  ...
  iN_Vector* cv_ewt;     /* error weight vector                          */
  iN_Vector* cv_y;       /* y is used as temporary storage by the solver */
                       /* The memory is provided by the user to CVode  */
                       /* where the vector is named yout.              */
  iN_Vector* cv_acor;    /* In the context of the solution of the        */
                       /* nonlinear equation, acor = y_n(m) - y_n(0).  */
                       /* On return, this vector is scaled to give     */
                       /* the estimated local error in y.              */
  iN_Vector* cv_tempv;   /* temporary storage vector                     */
  iN_Vector* cv_ftemp;   /* temporary storage vector                     */
  ...
  int (*cv_lsetup)(struct CVodeMemRec *cv_mem, int convfail, iN_Vector* ypred,
		   iN_Vector* fpred, bool *jcurPtr, iN_Vector* vtemp1,
		   iN_Vector* vtemp2, iN_Vector* vtemp3); 

  int (*cv_lsolve)(struct CVodeMemRec *cv_mem, iN_Vector* b, iN_Vector* ycur,
		   iN_Vector* fcur);
