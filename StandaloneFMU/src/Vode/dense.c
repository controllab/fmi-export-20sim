...
integer *DenseAllocPiv(integer N)
{
  ...
  /* grn: 2013-02-28 initialize with zeros now */
...
void DenseBacksolve(DenseMat *A, integer *p, iN_Vector* b)
{
...
void DenseCopy(DenseMat *A, DenseMat *B)
{
	if( A == NULL || B == NULL )
	{
		...
		// the case where the source has a value and the destination not is not allowed.
		//ASSERT(A == NULL);
		return;
	}
...
real **denalloc(integer n)
{
  ...
  /* grn: 2013-02-28 initialize with zeros now */
  /* this allocates the full matrix in the first element ! */
  /* in this way only one memory allocation has to be done */
  a[0] = (real *) calloc(n * n, sizeof(real));
  if (a[0] == NULL) {
    free(a);
    return(NULL);
  }

  /* grn: map the rest of the matrix to the other elements */
  for (j=1; j < n; j++)
		a[j] = a[0] + j * n;
...
#else
	/* grn: this will do the same, since the first element holds
	   the whole matrix allocated */
